...
and equational reasoning principles.
...
In the free model, equality is decided by computing normal forms and comparing them syntactically. Equational_Reasoning_in_∞-Categ…

Repository Structure
.
├── FreeKernel.agda        # Core normalization and free model machinery
├── (other modules)        # Supporting syntax, typing, and rewriting infrastructure
└── Equational_Reasoning_in_∞-Categories.pdf  # Reference paper
...
and the kernel of the decision procedure for equality in the free ∞-category.
...
These rules provide a symbolic calculus that can be interpreted either strictly (as equalities) or weakly (as equivalences/homotopies). Equational_Reasoning_in_∞-Categ…

Free Model and Normalization
Given a stratified set of generators (cells with assigned dimensions), the free ∞-category is constructed as well-formed terms modulo provable equations. Equational_Reasoning_in_∞-Categ…
A key feature formalized here is:

A deterministic normalization algorithm that computes normal forms of terms; two terms are equal in the free model iff their normal forms coincide. Equational_Reasoning_in_∞-Categ…
...
and a constructive basis for mechanized higher-categorical reasoning.
...
This duality allows one to derive symbolic equations in the strict mode and interpret them homotopically in the weak mode without changing the syntax. Equational_Reasoning_in_∞-Categ…
...
(Additional include paths may be required depending on your local Agda setup.)
